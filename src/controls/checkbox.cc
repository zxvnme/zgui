...
void zgui::checkbox(const char* id, bool& value) noexcept
{
	...
	if (const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width + 6 + text_wide, control_height); !active && hovered && utils::input::key_pressed(VK_LBUTTON))
	{
	...
	utils::misc::push_cursor_pos(vec2{ cursor_pos.x + 14 + text_wide + global_config.item_spacing, cursor_pos.y });

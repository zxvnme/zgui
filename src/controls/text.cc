...
bool zgui::clickable_text(const char* id) noexcept
{
	...
	const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, text_width, text_tall);
	...
	utils::misc::push_cursor_pos(vec2{ cursor_pos.x, cursor_pos.y + text_tall / 2 + global_config.item_spacing });
